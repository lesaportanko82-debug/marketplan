...
export function ProjectDetail() {
  ...
  useEffect(() => {
    getData<Project[]>(PROJECTS_KEY).then((d) => {
      ...
      setProject(mockProjects.find((p) => p.id === id) || null);
    });
  }, [id]);
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="border-b border-border bg-card px-5 py-3">
        <div className="max-w-[1440px] mx-auto">
          <button
            onClick={() => navigate("/")}
            className="flex items-center gap-1.5 text-muted-foreground hover:text-foreground text-[14px] mb-3 transition-colors"
          >
...
function SummaryContent({ project }: { project: (typeof mockProjects)[0] }) {
  const formatNum = (n: number) =>
    n >= 1000000
      ? `${(n / 1000000).toFixed(1)}M`
      : n >= 1000
